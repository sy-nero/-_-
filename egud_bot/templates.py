...
def _grant(business_name, association_name, sender_name,
           field="", neighborhood="", **_):
    saw = _saw_clause(field, neighborhood)
    subject = (f"{business_name}, לגבי מענק מחשוב לעסק" if business_name
               else "לגבי מענק מחשוב לעסקים")
    text = (
        f"שלום,\n\n"
        f"שמי {sender_name}, מ{association_name}.\n\n"
        ...
        f"מכירות, מלאי, תוכנה וציוד{saw}\n\n"
        f"אנחנו מרכזים עכשיו קבוצה של בעלי עסקים מהמגזר שמגישים יחד, וככל שהקבוצה "
        f"גדולה יותר, כך התנאים והליווי שכולנו מקבלים טובים יותר. המועד האחרון "
        f"להגשה הוא {GRANT_DEADLINE}.\n\n"
        f"אם זה מעניין אתכם, תשיבו לי לכאן עם שם וטלפון ואחזור אליכם.\n\n"
        f"תודה,\n{sender_name}\n{association_name}\n"
    )
    p = "margin:0 0 14px;"
    html = f"""<!DOCTYPE html>
...
<body style="margin:0;padding:0;background:#ffffff;">
  <div dir="rtl" style="direction:rtl;text-align:right;max-width:600px;margin:0 auto;
    ...
    <p style="{p}">יש עכשיו מענק ממשלתי חדש למחשוב ואוטומציה בעסקים
      ...
      שקל, על מערכות ניהול, מכירות, מלאי, תוכנה וציוד{saw}</p>
    <p style="{p}">אנחנו מרכזים עכשיו קבוצה של בעלי עסקים מהמגזר שמגישים יחד, וככל
      שהקבוצה גדולה יותר, כך התנאים והליווי שכולנו מקבלים טובים יותר. המועד האחרון
      להגשה הוא {GRANT_DEADLINE}.</p>
    <p style="{p}">אם זה מעניין אתכם, תשיבו לי לכאן עם שם וטלפון ואחזור אליכם.</p>
    <p style="margin:0 0 4px;">תודה,</p>
    <p style="margin:0;">{sender_name}<br>{association_name}</p>
  </div>
</body></html>"""
