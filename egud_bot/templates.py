...
def build_html(
    ...
    benefits = (
        _benefit_row("₪", "משלמים פחות",
                     "עלויות וריביות נמוכות יותר, כי כוח הקבוצה עובד בשבילך ולא נגדך.")
        + _benefit_row("✦", "יותר סיכוי לאישור",
                       "סיכויים גבוהים בהרבה לקבל את המימון שאתה צריך, ובמהירות.")
        + _benefit_row("♦", "הנחות ותנאים בלעדיים",
                       "הטבות, הנחות ותנאים שמורים לחברי הקבוצה בלבד.")
    )
        ...
        <tr>
          <td style="padding:32px 34px 4px;">
            ...
            <p style="margin:0 0 12px;font-size:18px;line-height:1.8;color:{TEXT_2};">
              אתה עובד קשה על העסק, ובכל פעם שאתה צריך מימון אתה מגלה שאתה
              <strong style="color:{TEXT};">משלם יותר מדי</strong>, ועדיין לא בטוח שתקבל אישור
              או תנאים טובים.
            </p>
            <p style="margin:0 0 12px;font-size:18px;line-height:1.8;color:{TEXT_2};">
              בקבוצה של <strong style="color:{TEXT};">איגוד העסקים החרדיים</strong> אתה
              <strong style="color:{TEXT};">משלם פחות</strong>, ומגדיל משמעותית את הסיכויים שלך
              לקבל אישור מימון, הנחות ותנאים שלא היית משיג לבד.
            </p>
          </td>
        </tr>
...
def build_text(
    ...
    return (
        ...
        f"{greeting}\n\n"
        f"אתה עובד קשה על העסק, ובכל פעם שאתה צריך מימון אתה מגלה שאתה משלם יותר מדי, "
        f"ועדיין לא בטוח שתקבל אישור או תנאים טובים.\n\n"
        f"בקבוצה של {association_name} אתה משלם פחות, ומגדיל משמעותית את הסיכויים שלך "
        f"לקבל אישור מימון, הנחות ותנאים שלא היית משיג לבד.\n\n"
        f"מה תקבלו בקבוצה:\n"
        f"  • משלמים פחות, עלויות וריביות נמוכות יותר\n"
        f"  • יותר סיכוי לאישור המימון שאתה צריך\n"
        f"  • הנחות ותנאים בלעדיים לחברי הקבוצה\n\n"
        f"להשארת פרטים:\n{link}\n"
    )
