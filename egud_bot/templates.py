...
def _grant(business_name, association_name, sender_name,
           field="", neighborhood="", **_):
    # פנייה אישית לפי תחום/עיר (בלי שם בעלים): "אני מחפשת... ונראה לי שאתה מתאים כי"
    where = f" ב{neighborhood}" if neighborhood else ""
    if field:
        reason = f"כי {field} כמו שלך{where} בדיוק יכול להרוויח משדרוג דיגיטלי"
    else:
        reason = "כי זה בדיוק סוג העסק שהמענק הזה נועד לו"
    personal = (f"אני מחפשת כמה עסקים שיכולים להרוויח ממשהו טוב שמצאתי, "
                f"ונראה לי שדווקא אתה מתאים, {reason}.")
    hi = "היי,"
    subject = "שאלה קטנה אליך"
    text = (
        f"{hi}\n"
        f"אני מירי, בעלת עסק בעצמי.\n\n"
        f"{personal}\n\n"
        f"היום כמעט כל עסק צריך מערכות מסודרות לניהול, מלאי ומכירות כדי לחסוך זמן, "
        f"לעבוד חכם יותר ולגדול, אבל שדרוג כזה עולה כסף. יש עכשיו מענק ממשלתי "
        f"({GRANT_NAME} של משרד הכלכלה) שבו המדינה מחזירה עד 35 אחוז מהעלות, עד "
        f"350 אלף שקל, בדיוק על הדברים האלה: מערכות ניהול, מכירות, מלאי, תוכנה וציוד. "
        f"זו הזדמנות לשדרג את העסק בלי לשאת לבד בכל העלות.\n\n"
        ...
        f"פשוט תשיב לי למייל הזה עם השם והטלפון שלך, ואחזור אליך עם כל הפרטים.\n\n"
        f"{sender_name}\n"
    )
    p = "margin:0 0 14px;"
    html = f"""<!DOCTYPE html>
...
<body style="margin:0;padding:0;background:#ffffff;">
  <div dir="rtl" style="direction:rtl;text-align:right;max-width:600px;margin:0 auto;
       ...
       line-height:1.75;color:#222222;">
    <p style="{p}">{hi}</p>
    <p style="{p}">אני מירי, בעלת עסק בעצמי.</p>
    <p style="{p}">{personal}</p>
    <p style="{p}">היום כמעט כל עסק צריך מערכות מסודרות לניהול, מלאי ומכירות כדי
      לחסוך זמן, לעבוד חכם יותר ולגדול, אבל שדרוג כזה עולה כסף. יש עכשיו מענק
      ממשלתי ({GRANT_NAME} של משרד הכלכלה) שבו המדינה מחזירה עד 35 אחוז מהעלות,
      עד 350 אלף שקל, בדיוק על הדברים האלה: מערכות ניהול, מכירות, מלאי, תוכנה
      וציוד. זו הזדמנות לשדרג את העסק בלי לשאת לבד בכל העלות.</p>
    ...
    <p style="{p}">המועד האחרון להגשה הוא {GRANT_DEADLINE}, אז נשאר מעט זמן. אם בא
      לך להצטרף, פשוט תשיב לי למייל הזה עם השם והטלפון שלך, ואחזור אליך עם כל
      הפרטים.</p>
