...
## אפליקציית ניהול הקמפיינים

```bash
python webapp/app.py
```
ואז http://localhost:5001

לוח שמחליף את גיליון הקמפיינים: כל קמפיין הוא עמודה, וכל שורה בו
(נוסח, קהל יעד, מקום חיפוש, מאפיין, כאב, הנעה לפעולה) מוזנת עם
**פרטים / הערות / המלצות לשיפור**, בדיוק כמו בגיליון.

- **שליחה מתוך האפליקציה** — כל מייל מוצג במלואו, מסמנים למי לשלוח, ושולחים.
- **מדידה אוטומטית** — כמות פניות, כמות פתיחות והודעה חוזרת נמדדות מהמיילים
  עצמם ולא מוזנות ביד. שלבי המשפך שקורים בשיחה (התאמה, פגישת מו״מ, הסכמה
  עקרונית, חתימה, הפניות בפועל) מוזנים ידנית.
- **מעקב פתיחות** — כל מייל נשלח עם פיקסל שקוף, וכשהנמען פותח אותו נרשמת
  פתיחה.

### העלאה לענן

מדריך מלא: **[docs/deploy.md](docs/deploy.md)**. בקצרה — `render.yaml`
בפרויקט מגדיר שירות עם דיסק קבוע, ו-`Dockerfile` מאפשר פריסה לכל שירות
קונטיינרים. חובה להגדיר `APP_PASSWORD`: בענן האפליקציה חשופה לכל מי שיש לו
את הכתובת, והיא מציגה לידים ושולחת מיילים. העברת הנתונים מהמחשב לענן
נעשית דרך עמוד "גיבוי" שבאפליקציה (הורדת zip והעלאתו בצד השני).

### מה נדרש כדי שמעקב הפתיחות יעבוד

`TRACKING_BASE_URL` ב-`.env` חייב להיות כתובת של האפליקציה **שנגישה
מהאינטרנט**. פיקסל שמצביע ל-localhost לא ייטען אצל אף נמען, ואז כמות
הפתיחות תישאר 0 לנצח. בלי הגדרה כזאת המיילים נשלחים כרגיל, פשוט בלי מדידת
פתיחות.

גם עם כתובת תקינה, המספר הוא הערכה בלבד: תוכנות מייל רבות חוסמות תמונות
(פתיחה כזאת לא נספרת), ו-Gmail טוענת תמונות דרך מטמון שלה. כלומר המספר
האמיתי גבוה מהמוצג — הוא שימושי להשוואה בין נוסחים, לא כאמת מוחלטת.
